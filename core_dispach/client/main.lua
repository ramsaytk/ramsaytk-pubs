...
function addBlipsForUnits()
    QBCore.Functions.TriggerCallback("core_dispach:getUnits", function(units)
...
function openDispach()
    if Config.JobOne.job == job or Config.JobTwo.job == job or Config.JobThree.job == job then
        ...
        QBCore.Functions.TriggerCallback( "core_dispach:getInfo", function(units, calls, ustatus)
...
AddEventHandler("core_dispach:callAdded", function(id, call, j, cooldown, sprite, color)
...
RegisterNetEvent("core:dispach:playerdata")
AddEventHandler("core:dispach:playerdata", function(PlayerData)
